...
export const Timeline: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-background border-t border-gray-800 overflow-hidden">
      {/* Timeline Toolbar */}
      <div className="h-10 border-b border-gray-800 flex items-center justify-between px-4 bg-surface">
        <div className="flex items-center space-x-4">
          ...
          <button
            ...
            className="p-1.5 hover:bg-surface-light rounded disabled:opacity-30 text-gray-300 transition-colors"
            title="Split (S)"
          >
            <Scissors size={16} />
          </button>
          <button
            ...
            disabled={!selectedClipId}
            className="p-1.5 hover:bg-red-900/30 hover:text-red-400 rounded disabled:opacity-30 text-gray-300 transition-colors"
            title="Delete (Del)"
          >
            <Trash2 size={16} />
          </button>
        ...
        <div className="flex items-center space-x-2">
          <button onClick={() => setZoom(Math.max(1, zoom - 2))} className="p-1 hover:bg-surface-light rounded transition-colors">
            <Minus size={16} />
          </button>
          <span className="text-[10px] text-gray-500 w-8 text-center">{zoom}x</span>
          <button onClick={() => setZoom(Math.min(100, zoom + 2))} className="p-1 hover:bg-surface-light rounded transition-colors">
            <Plus size={16} />
          </button>
        </div>
      </div>
      ...
      >
        ...
        <div
          ...
          style={{ left: 96 + currentTime * zoom }}
        >
        ...
        <div className="h-8 border-b border-gray-800 relative bg-surface-light" style={{ width: 96 + duration * zoom }}>
          ...
          <div className="ml-24 relative h-full">
            {Array.from({ length: Math.ceil(duration) }).map((_, i) => (
              <div
                key={i}
                className="absolute top-0 border-l border-gray-700 h-full flex flex-col justify-end pb-1 pl-1"
                style={{ left: i * zoom }}
              >
        ...
        <div className="min-w-full" style={{ width: 96 + duration * zoom }}>
          {tracks.map((track) => (
            <div
              key={track.id}
              className={`h-16 border-b border-gray-800 relative group transition-colors ${
                track.type === 'video' ? 'bg-black/10' : 'bg-black/20'
              }`}
